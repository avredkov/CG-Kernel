...
class PreprocessConfig(BaseModel):
    target_column: str = Field(default="Growth rate")
    ...
    min_target_value: Optional[float] = Field(default=None)
    log_target: bool = Field(default=False)
...
class DataPreprocessor:
    """Subset of the original preprocessor sufficient for inference.

    This class mirrors the attributes used at inference time:
    - config: PreprocessConfig
    - scaler: fitted StandardScaler
    - regime_levels_: list of categorical levels
    - feature_names_: list[str]
    Methods: transform (used for inference only)
    """
    ...
    def transform(self, df: pd.DataFrame) -> Tuple[np.ndarray, pd.Series, pd.Series, List[str]]:
        if self.scaler is None or self.regime_levels_ is None:
        ...
        X = np.concatenate([cont_arr, cat_arr], axis=1)
        # For inference we don't use y; return placeholder series for compatibility
        y = pd.Series(np.zeros(len(df), dtype=np.float32))
        return X.astype(np.float32), y.reset_index(drop=True), regimes.reset_index(drop=True), (self.feature_names_ or [])
